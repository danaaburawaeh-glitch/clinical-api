...
def test_relaxation_budget_is_configured_below_action_timeout():
    """GPT Actions abandon a call around 45s. The widening budget must
    leave room for the strict round plus post-processing, otherwise a
    helpful widening turns into a client-side timeout that the clinician
    sees as 'technical failure' — strictly worse than an honest empty
    result."""
    from app.settings import get_settings

    settings = get_settings()
    assert settings.relaxation_budget_seconds > 0
    assert settings.relaxation_budget_seconds <= 20, (
        "budget too large: widening could outlive the GPT Action timeout"
    )
